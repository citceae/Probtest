...
对于存在自环的图可能有问题？存在单纯的自指结构吗？我认为不存在（改！
...
结构存在问题？可能是entry的问题 应该处理为独立函数XXXX
是if... else if...else 存在问题，entry会同时和if以及else if分支连接
或者有可能是条件判断里的&&的问题因为有lhs什么的 看起来是因为这个！（这一坨屎写得我好烦躁啊orz）

或者可以不分割最大的而是也按reward的概率做分割？分割的去重（即不对单元素区间再次分割）

程序通过静态分析获取静态控制流图，通过输入语法获取相关输入信息
采样策略 随便画了个流程图orz

如何纠正算法的非预期行为？如何对抗随机性？
设定最小区间？参数K，k? reward?拉普拉斯近似中减少0的光滑程度（比如设定成#（s，t）+\delta(x)）？改为0.05权重，增加了有效样本的作用


!!!if elseif的问题是：存在if_end节点不对应程序所在行 当前的MulCFGPass必须以行号作为标记因此存在问题 TODO

增加dsampleing方法 每次只利用当前采样的结果 可以精简化测试数量？无法保证最后一次得到的能够有好的性质